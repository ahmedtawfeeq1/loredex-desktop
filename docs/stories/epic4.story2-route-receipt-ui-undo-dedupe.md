...
- 2026-07-11 fresh-eyes (commit `4a30042`): **PASS.** Routing safety complete across stories 4-1…4-4: receipt+Undo (`route()` returns receiptId; `route.undo` → `undoRoute`, byte-identical restore, fails loudly `ROUTE_ALREADY_UNDONE`/`ROUTE_RECEIPT_NOT_FOUND`; toast reversible), dedup source (`routeHistory`/`listReceipts`), filing-scope never-route globs (`scopeBlock`/`matchNeverRoute`, `route.preview` short-circuits `ROUTE_BLOCKED`, persisted via lib `saveConfig` = CLI-honored, `ScopeSettings.tsx`), drift badge + reroute (`noteDrift` live-body-hash vs stamped `source_hash`, `DriftBadge.tsx` in `NoteView`, reroute reuses `route()`). Tests `route-safety.test.ts` / `route.test.ts` / `engine.test.ts` green in 933/933.
